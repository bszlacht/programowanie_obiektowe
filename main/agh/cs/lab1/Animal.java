...
public class Animal implements IMapElement {
    ...
    LinkedList<IPositionChangeObserver> observersList = new LinkedList<>(); // to powinno być prywatne i może być finalne
    //konstruktor:
    ...
    public void move(MoveDirection direction) throws IllegalArgumentException {
        Vector2d tworzacyPozycje = new Vector2d(0, 0);   // polglish
        switch (direction) {
        ...
        if (this.map.canMoveTo(newPosition)) {
            this.positionChanged(this.getPosition(), newPosition);   // nazwa sugeruje, że notyfikacja jest już po zmianie pozycji
    ...
    void positionChanged(Vector2d oldPosition, Vector2d newPosition) {   // raczej private
