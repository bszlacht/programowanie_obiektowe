...
public class DNA {
    ...
    public DNA() {
        ...
        while (!validness) {
            ...
            //checking if valid:
            validness = this.isValid(); // czy ta zmienna coś wnosi? Jakby Pan wywołał metodę w warunku pętli?
        }
    ...
    public DNA(Animal father, Animal mother) {   // nie wystarczą DNA?
        Random rand = new Random();
        int[] groups = new int[32];  // 0,2,lub 3
        // 0 -> mother
        // 1 or 2 -> father
        for (int i = 0; i < 32; i++) {
            groups[i] = rand.nextInt(3);    // nie tak miało wyglądać krzyżowanie genów
        }
        for (int i = 0; i < 32; ++i) {
            if (groups[i] == 0)
        ...
        while (!this.isValid()) {
            ...
            int[] missingGenes = new int[8];    // myląca nazwa
            for (int i = 0; i < 32; i++) {
    ...
    private void fixParticularGene(int idx, int targetGen) { // czy ta metoda cokolwiek wnosi?
    ...
    public int getRandomGene() { // using for example to change direction
        return this.genes[(new Random()).nextInt(32)];  // nowy obiekt co wywołanie
    }
